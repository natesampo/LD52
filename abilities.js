class Ability extends UIButton {
	...
	render(screen) {
		...
		for (var j=0; j<rays; j++) {
			let procession = arcSize * j + this.spinCounter * spinSpeed;
			context.beginPath();
		...
		context.fillText(this.name, centerX, window.innerHeight/2.4 - 2.9 * this.img.height);
		context.strokeText(this.name, centerX, window.innerHeight/2.4 - 2.9 * this.img.height);
		...
		let linesRequired = lines.length - 1 + (this.cooldown != null) + (this.duration != null) + (this.damage != null);
		...
		let boxTopY = window.innerHeight/2.4 + 2.6 * this.img.height - margin;
		let boxBottomY = window.innerHeight/2.4 + 2.6 * this.img.height + 54 + 30 * linesRequired + margin;
		...
		for (var j=0; j<lines.length; j++) {
			context.fillText(lines[j], centerX, window.innerHeight/2.4 + 2.6 * this.img.height + 30 * j);
		}

		let linesWritten = 0;
		if (this.damage) {
			context.fillText('Damage: ' + this.damage, centerX, window.innerHeight/2.4 + 2.6 * this.img.height + 60 + 30 * (linesWritten + lines.length - 1));
			linesWritten++;
		}
		if (this.duration) {
			context.fillText('Duration: ' + this.duration + 's', centerX, window.innerHeight/2.4 + 2.6 * this.img.height + 60 + 30 * (linesWritten + lines.length - 1));
			linesWritten++;
		}
		if (this.cooldown) {
			context.fillText('Cooldown: ' + this.cooldown + 's', centerX, window.innerHeight/2.4 + 2.6 * this.img.height + 60 + 30 * (linesWritten + lines.length - 1));
			linesWritten++;
		}
	}
}
